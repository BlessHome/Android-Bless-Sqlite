...
import com.bless.sqlite.db.DataBase;
...
import com.bless.sqlite.db.assit.SQLiteHelper;
 ...
 */
public abstract class SQLite extends SQLiteClosable implements DataBase {
    ...
    protected SQLite otherDatabase;

    protected SQLite(SQLite dataBase) {
    ...
    protected SQLite(DataBaseConfig config) {
    ...
    private void initDatabasePath(String path) {
        Log.i(TAG, "create  database path: " + path);
        path = mConfig.context.getDatabasePath(mConfig.dbName).getPath();
        Log.i(TAG, "context database path: " + path);
        File dbp = new File(path).getParentFile();
        if (dbp != null && !dbp.exists()) {
            boolean mks = dbp.mkdirs();
            Log.i(TAG, "create database, parent file mkdirs: " + mks + "  path:" + dbp.getAbsolutePath());
        }
    }

    /**
     * get and new a operator based on SQLite
     *
     * @param context
     * @return {@link DSQLiteFactory}
     */
    public static SQLite newInstance(Context context) {
        return newInstance(context, DataBaseConfig.DEFAULT_DB_NAME);
    }
     ...
     * @return {@link DSQLiteFactory}
     */
    public static SQLite newInstance(Context context, String dbName) {
     ...
     * @return {@link DSQLiteFactory}
     */
    public static SQLite newInstance(DataBaseConfig config) {
        return newCascadeInstance(config);
    }
     ...
     */
    public static SQLite newSingleInstance(Context context, String dbName) {
     ...
     */
    public synchronized static SQLite newSingleInstance(DataBaseConfig config) {
        return DSQLiteFactory.newSingleInstance(config).single();
    }
     ...
     * @return {@link SingleSQLiteImpl}
     */
    public static SQLite newCascadeInstance(Context context, String dbName) {
     ...
     * @return {@link CascadeSQLiteImpl}
     */
    public synchronized static SQLite newCascadeInstance(DataBaseConfig config) {
        return DSQLiteFactory.newCascadeInstance(config).cascade();
    }
     ...
     * @return {@link CascadeSQLiteImpl}
     */
    public abstract SQLite cascade();
    ...
    public boolean deleteDatabase() {
        ...
        justRelease();
        Log.i(TAG, "data has cleared. delete Database path: " + path);
